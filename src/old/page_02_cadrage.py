...
def page_cadrage():
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            refs_prod = [
                ("Lindas, Goude & Ciais (2025)", "RTE + ERA5 → benchmark ML production solaire/éolienne · EDF R&D / CEA"),
                ("De Giorgi et al. (2014)", "Prévision PV par méthodes statistiques"),
                ("Malvoni et al.", "Prédiction PV méditerranéen via irradiance et température"),
                ("IRENA (2020)", "Revue des approches avancées de prévision ENR"),
            ]
            for ref, desc in refs_prod:
                color_bg = "#e8f4fd" if "Lindas" in ref else "#eaf4ff"
                st.markdown(f"""
                <div style="margin-bottom:0.4rem; padding:0.4rem 0.8rem; background:{color_bg};
            ...
            refs_vol = [
                ("Kiesel & Paraschiv (2017)", "Volatilité prix induite par les ENR"),
                ("Kulakov (2019)", "Modèles à régimes pour la volatilité renouvelable"),
                ("Cramer et al. (2022)", "Précision météo et volatilité intraday"),
                ("ACER (2023)", "Volatilité croissante avec la pénétration ENR"),
                ("Fraunhofer ISE (2022)", "Variabilité de la charge résiduelle"),
            ]
            for ref, desc in refs_vol:
                st.markdown(f"""
                <div style="margin-bottom:0.4rem; padding:0.4rem 0.8rem; background:#fff3cd;
        ...
        with col2:
            st.markdown("**Lindas et al. (2025) — EDF R&D / CEA**")
            st.markdown("""
<div style="padding:0.8rem 1rem; background:#e8f4fd; border-left:4px solid #0f3460;
...
- Arbres extrapolent mal quand la capacité croît → justifie le <strong>TCH</strong>
            ...
            manques = [
                ...
                ("Seuil de criticité calibré", "Pas de quantification du niveau critique pour le réseau"),
...
<div style="display:flex; gap:0.8rem; margin:0.5rem 0; flex-wrap:wrap; align-items:stretch;">
    ...
    <div style="flex:1.3; min-width:200px; padding:0.8rem 1rem; background:#eafaf1;
        ...
        <div style="color:#444; font-size:0.82rem;"><strong>Anticipe</strong> la variabilité à 30 min<br>Seuil calibré · Données ouvertes</div>
    </div>
    <div style="display:flex; align-items:center; font-size:1.4rem; color:#ccc; padding:0 0.3rem;">←</div>
    <div style="flex:1; min-width:160px; padding:0.8rem 1rem; background:#eaf4ff;
        ...
        """, unsafe_allow_html=True)
        callout("""
Lindas et al. (2025) valide notre approche RTE + météo spatiale.
<strong>Leur travail prédit des niveaux. Le nôtre prédit la variabilité — le problème le moins étudié.</strong>
        """, "info")
        st.markdown("</div>", unsafe_allow_html=True)
...
- **Variabilité météo significative** — contrastes littoral / vallées / massifs alpins
...
<strong>Variables explicatives (CAMS, météo)</strong> — haute résolution <strong>spatiale</strong>
            ...
            """, unsafe_allow_html=True)
            st.markdown("""
**Question centrale :** où collecter les données météo ?

→ Identifier des points géographiques représentatifs pondérés par le parc PV installé
            """)
            callout("Le clustering n'était pas prévu initialement — c'est la contrainte des données RTE qui l'a rendu nécessaire.", "warn")
            st.markdown("</div>", unsafe_allow_html=True)

        # ── Démarche de clustering en 5 étapes ──
        st.markdown("<div class='slide-section'><h2>Démarche de clustering</h2>", unsafe_allow_html=True)
        steps = [
            ("1", "Registre ODRÉ", "Installations PV PACA avec énergie injectée par commune"),
            ("2", "Nettoyage", "Exclusion entrées sans commune et NaN énergie"),
            ("3", "KMeans (lat, lon)", "K=5 — représentativité territoriale + volume maîtrisé"),
            ("4", "Centroïdes énergie", "Moyenne pondérée par énergie injectée"),
            ("5", "Commune la + proche", "Point représentatif par cluster"),
        ]
        cols = st.columns(5)
        for i, (num, title, desc) in enumerate(steps):
            with cols[i]:
                st.markdown(f"""
                <div style="text-align:center; padding:0.8rem 0.5rem; background:#f8f9fd;
                            border-radius:10px; border-top:3px solid #0f3460; height:150px;">
    ...
    with tabs[2]:
        ...
        with col1:
            ...
            st.markdown("""
| Source | Variables | Licence |
|---|---|---|
| Registre ODRÉ | `energieannuelleglissanteinjectee`, `codeinseecommune` | Etalab OL v2 |
| INSEE communes | `code_insee`, `lat`, `lon`, `nom` | ODbL |
            """)
            callout("Choix de l'énergie <strong>injectée</strong> (vs produite) : taux de NaN plus faible, mesure directement observable.", "info")
            st.markdown("</div>", unsafe_allow_html=True)

            st.markdown("<div class='slide-section'><h2>Données manquantes</h2>", unsafe_allow_html=True)
            cleaning = [
                ("Communes sans code INSEE", "45 lignes — 0,25 % énergie", "#f39c12"),
                ("Énergie injectée manquante", "149 lignes — 1,70 % puissance", "#f39c12"),
                ("Énergie négative", "Erreurs de saisie — exclues", "#e94560"),
                ("Doublons", "Vérifiés et supprimés", "#1abc9c"),
            ]
            for label, detail, color in cleaning:
                st.markdown(f"""
                <div style="margin-bottom:0.4rem; padding:0.4rem 0.8rem;
                            border-left:3px solid {color}; background:#f8f9fd; border-radius:6px;">
                    <span style="font-weight:700; color:#1a1a2e; font-size:0.88rem;">{label}</span>
                    <span style="color:#555; font-size:0.82rem;"> — {detail}</span>
                </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with col2:
            ...
            callout("""
<strong>K = 5 : choix pragmatique.</strong> Le coude ne tranche pas nettement —
K=5 garantit couverture territoriale + volume de données maîtrisé.
            """, "warn")
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with col1:
            ...
            for i, row in COMMUNES.iterrows():
                fig_map.add_trace(go.Scattergeo(
                    ...
                    mode="markers+text",
                    marker=dict(size=18, color=COLORS[i], symbol="star",
        ...
        with col2:
            ...
            for i, row in COMMUNES.iterrows():
                st.markdown(f"""
                <div style="margin-bottom:0.7rem; padding:0.8rem 1rem; border-radius:10px;
                            border-left:4px solid {COLORS[i]}; background:#f8f9fd;">
                    <div style="font-weight:700; color:{COLORS[i]}; font-size:0.95rem;">
                        Cluster {i} — {row['commune']}</div>
                    <div style="color:#555; font-size:0.82rem; margin-top:0.2rem;">
                        {row['zone']} · <strong>{row['contrib_pct']} %</strong>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        callout("""
<strong>5 points d'ancrage</strong> pour toute la suite : chaque variable météo, atmosphérique
et astronomique sera collectée à ces 5 localisations puis agrégée au niveau régional.
        """, "success")
