    ...
    with tabs[0]:
        ...
        with col1:
            metrics_data = {
                ...
                "Rôle": ["⭐ Principal", "Secondaire", "Secondaire", "Secondaire", "⚠️ Régularité"]
            }
...
La **baseline** prédit que la variabilité future sera proche de la dernière variation observée :
        ...
        \qquad \longleftarrow \text{variable : dtch\_solaire\_abs\_dt}
        $$
        """)
...
**Lasso** : peut mettre des coefficients à 0 → sélection implicite
        ...
        st.markdown("---")
        st.markdown("### Résultats comparatifs — validation 2024")

        results = pd.DataFrame([
            {"Modèle": "Naïf (baseline)", "MAE_valid": "0.90", "NMAE_valid (%)": "~38%", "R2_valid": "0.76",
             "Gap_MAE": "0.05"},
            {"Modèle": "Régression linéaire simple", "MAE_valid": "1.08", "NMAE_valid (%)": "~45%","R2_valid": "0.62",
             "Gap_MAE": "0.05"},
            {"Modèle": "Régression linéaire multi", "MAE_valid": "0.64", "NMAE_valid (%)": "~27%", "R2_valid": "0.88",
             "Gap_MAE": "0.02"},
            {"Modèle": "Ridge (optimal α)", "MAE_valid": "0.63", "NMAE_valid (%)": "~26%", "R2_valid": "0.88",
             "Gap_MAE": "0.03"},
            {"Modèle": "Lasso (optimal α)", "MAE_valid": "0.63", "NMAE_valid (%)": "~26%", "R2_valid": "0.88",
             "Gap_MAE": "0.03"},
        ])
...
Random Forests / Extra Trees / Gradient Boosting (LGBM, XGBoost, HistGB) / SVR / K-Nearest Neighbors ...
        ...
        lazy_results = pd.DataFrame({
            ...
            "R²": [0.916, 0.913, 0.913, 0.911, 0.904, 0.903, 0.896, 0.867, 0.824],
            "MAE": [0.465, 0.475, 0.493, 0.496, 0.509, 0.546, 0.549, 0.669, 0.674],
        })
